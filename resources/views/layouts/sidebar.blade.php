...
<aside class="{{ $mobile ? 'flex h-full w-full max-w-[18rem] flex-col bg-slate-900 text-white shadow-2xl' : 'hidden md:flex md:flex-col fixed left-0 top-0 h-screen w-64 bg-slate-900 text-white shadow-md z-50' }}">
    ...
    <nav class="flex-1 px-3 pt-4 pb-4 space-y-1 overflow-y-auto">
        ...
        @endif
        <a href="{{ route('admin.attendance') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 @if(request()->routeIs('admin.attendance')) bg-emerald-600 text-white @else text-slate-300 hover:text-white hover:bg-slate-800 @endif">
