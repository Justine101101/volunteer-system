    ...
    <div class="py-8 bg-slate-50" data-animate="fade-up">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex flex-wrap items-center justify-between gap-4" data-animate="fade-up">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex items-center gap-2">
                        <a href="{{ route('events.calendar', ['year' => $prevMonth->year, 'month' => $prevMonth->month]) }}" 
                           class="p-2 rounded-lg hover:bg-emerald-50 transition">
                ...
                <div class="flex items-center gap-2">
                    <button class="p-2 rounded-lg hover:bg-emerald-50 transition" title="Messages">
                        <svg class="w-5 h-5 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path>
                        </svg>
                    </button>
                    <button class="p-2 rounded-lg hover:bg-emerald-50 transition" title="Calendar">
                        <svg class="w-5 h-5 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                        </svg>
                    </button>
                    <button class="p-2 rounded-lg hover:bg-emerald-50 transition" title="Print" onclick="window.print()">
                        <svg class="w-5 h-5 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path>
                        </svg>
                    </button>
                    <div class="relative">
                        <button class="px-4 py-2 bg-emerald-50 rounded-lg hover:bg-emerald-700 transition font-medium text-emerald-600 flex items-center gap-2">
                            Month
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden" data-animate="zoom-in">
                ...
                <div class="grid grid-cols-7 bg-emerald-50 border-b border-slate-200">
                    ...
                    @foreach($dayNames as $day)
                        <div class="p-3 text-center font-semibold text-emerald-600 text-sm">
                            {{ $day }}
                        </div>
                    @endforeach
                </div>
                    ...
                    @while($currentDate <= $endCalendar)
                        ...
                        <div class="min-h-24 border-r border-b border-gray-200 p-2 {{ !$isCurrentMonth ? 'bg-gray-50' : 'bg-white' }} hover:bg-gray-50 transition">
                            <div class="flex items-start justify-between mb-1">
                                <span class="text-sm font-medium {{ $isCurrentMonth ? 'text-gray-900' : 'text-gray-400' }} {{ $isToday ? 'bg-emerald-600 text-white rounded-full w-7 h-7 flex items-center justify-center' : '' }}">
                                    {{ $currentDate->day }}
                                </span>
                            ...
                            <div class="space-y-1 mt-1">
                                @foreach($dayEvents->take(2) as $event)
                                    <a href="{{ route('events.show', ['eventId' => $event->id]) }}" 
                                       class="block text-xs text-emerald-600 hover:text-emerald-700 transition"
                                       title="{{ $event->title }}">
