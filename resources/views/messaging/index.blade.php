            ...
            <div class="flex-1 flex-col bg-white {{ isset($otherUser) && isset($messages) ? 'flex' : 'hidden md:flex' }}">
                @if(isset($otherUser) && isset($messages))
                        ...
                        <div class="max-w-4xl mx-auto space-y-4">
                            @foreach($messages as $message)
                                    ...
                                    @if($message->sender_id === auth()->id())
                                        ...
                                        <div class="flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity duration-200 message-actions">
                                            <div class="flex flex-col gap-1">
                                                ...
                                                <form action="{{ route('messaging.destroy', $message) }}" method="POST" class="inline" data-confirm="Delete this message?">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button 
                                                        type="submit"
                                ...
                                @if($message->sender_id === auth()->id())
                                    ...
                                    <form id="edit-{{ $message->id }}" action="{{ route('messaging.update', $message) }}" method="POST" class="hidden max-w-[85%] sm:max-w-[70%] ml-auto mt-2">
                                        @csrf
                                        @method('PUT')
                    ...
                    <div class="px-3 sm:px-6 py-3 sm:py-4 border-t border-slate-200 bg-white shadow-lg">
                        <form action="{{ route('messaging.send') }}" method="POST" id="messageForm" class="flex items-end gap-2 sm:gap-3">
                            @csrf
