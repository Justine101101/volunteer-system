<x-app-layout>
            ...
            <div class="w-80 bg-white border-r border-slate-200 flex flex-col shadow-sm">
                ...
                <div class="flex-1 overflow-y-auto" id="conversationsList">
                    @forelse($conversations as $conversation)
                        <a 
                            href="{{ route('admin.messaging.chat', $conversation['user']->id) }}" 
                                    ...
                                    @if($message->sender_id === auth()->id())
                                        ...
                                        <div class="flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity duration-200 message-actions">
                                            <div class="flex flex-col gap-1">
                                                ...
                                                <form action="{{ route('admin.messaging.destroy', $message) }}" method="POST" class="inline" onsubmit="return confirm('Delete this message?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button 
                                                        type="submit"
    ...
    <div 
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <!-- Modal Body -->
            <form action="{{ route('admin.messaging.send') }}" method="POST" id="newMessageForm" class="flex-1 overflow-y-auto p-6">
                @csrf
                <div class="space-y-4">
                    <div class="relative">
