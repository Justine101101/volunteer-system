            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="p-6 sm:p-8">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="min-w-0">
                            <h3 class="text-lg font-semibold text-slate-900 truncate">{{ $user->name }}</h3>
                    ...
                    <div class="mt-10 border-t border-slate-100 pt-6">
                        <h4 class="text-sm font-semibold text-slate-900 mb-3">Event Participation</h4>
                        @if(!empty($participation))
                            <div class="space-y-3">
                                @foreach($participation as $item)
                                    <div class="flex items-start justify-between rounded-xl border border-slate-200 px-4 py-3 bg-slate-50/60">
                                        <div>
                        ...
                        @else
                            <p class="text-sm text-slate-500">This user has not joined any events yet.</p>
                        @endif
                    </div>

                    <div class="mt-8 flex items-center justify-between border-t border-slate-100 pt-6">
                        <a href="{{ route('admin.users.index') }}" class="text-sm text-slate-600 hover:text-slate-900">
                            ← Back to users
                        </a>
                        <div class="flex items-center gap-2">
                            <a href="{{ route('admin.users.edit', $user) }}" class="text-sm font-medium text-emerald-700 hover:text-emerald-800">
                                Edit user →
                            </a>
                        </div>
                    </div>
